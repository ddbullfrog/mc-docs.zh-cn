---
title: Data Protection Manager (DPM) 和 Azure 备份服务器 (MABS) 的脱机备份 - 早期版本
description: 借助 Azure 备份，可以使用 Azure 导入/导出服务在网外发送数据。 本文介绍了早期版本的 DPM 和 Azure 备份服务器的脱机备份工作流。
ms.topic: conceptual
author: Johnnytechn
ms.author: v-johya
ms.date: 09/28/2020
ms.openlocfilehash: 93e555d82f1982d9eb472b8bd78014c00221353e
ms.sourcegitcommit: 80567f1c67f6bdbd8a20adeebf6e2569d7741923
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 10/09/2020
ms.locfileid: "91871554"
---
...
Azure 备份的脱机种子设定过程与 [Azure 导入/导出服务](../storage/common/storage-import-export-service.md)紧密集成。 可以借助此服务使用磁盘将数据传输到 Azure。 如果要通过高延迟、低带宽网络传输 TB 计的初始备份数据，可以使用脱机种子设定工作流将一个或多个硬盘驱动器中的初始备份副本传送到 Azure 数据中心。 本文将概述如何对 System Center Data Protection Manager (DPM) 和 Azure 备份服务器 (MABS) 完成此工作流，并提供其他相关步骤。

> [!NOTE]
> Azure 恢复服务 (MARS) 代理的脱机备份过程与 DPM 和 MABS 的不同。 有关使用 MARS 代理进行脱机备份的信息，请参阅 [Azure 备份中的脱机备份工作流](backup-azure-backup-import-export.md)。 使用 Azure 备份代理完成的系统状态备份不支持脱机备份。
>
...
* 上述命令将创建文件 `C:\Program Files\Azure Recovery Services Agent\Scratch\MicrosoftBackupProvider\OfflineApplicationParams_<Storageaccountname>.xml`。
...
1. 转到注册表项 Computer\HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Azure Backup\Config\CloudBackupProvider。
1. 右键单击“CloudBackupProvider”并添加名称为 `AzureADAppCertThumbprint_<Azure User Id>` 的新字符串值。
    ...
    >* 转到注册表路径 `Computer\HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Azure Backup\DbgSettings\OnlineBackup; Name: CurrentUserId;`。
...
`*\\Azure Recovery Services Agent\Utils\*`
