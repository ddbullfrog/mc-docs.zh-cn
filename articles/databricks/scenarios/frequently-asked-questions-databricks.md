...
ms.topic: conceptual
ms.date: 10/25/2018
ms.openlocfilehash: 75f9f8113b7081359e429f1b44d8bc952e4c5e55
ms.sourcegitcommit: 93309cd649b17b3312b3b52cd9ad1de6f3542beb
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 10/30/2020
ms.locfileid: "93106686"
---
...
是的。 可以使用 Azure Key Vault 来存储要用于 Azure Databricks 的密钥/机密。 有关详细信息，请参阅 [Azure Key Vault 支持的作用域](/databricks/security/secrets/secret-scopes)。


## <a name="can-i-use-azure-virtual-networks-with-databricks"></a>是否可以将 Azure 虚拟网络与 Databricks 配合使用？
是的。 可以将 Azure 虚拟网络 (VNET) 与 Databricks 配合使用。 有关详细信息，请参阅[在 Azure 虚拟网络中部署 Azure Databricks](/databricks/administration-guide/cloud-configurations/azure/vnet-inject)。

## <a name="how-do-i-access-azure-data-lake-storage-from-a-notebook"></a>如何使用笔记本访问 Azure Data Lake Storage？ 

执行以下步骤：
1. 在 Azure Active Directory (Azure AD) 中预配服务主体并记录其密钥。
1. 在 Data Lake Storage 中分配服务主体的必需权限。
1. 若要访问 Data Lake Storage 中的文件，请使用 Notebook 中的服务主体凭据。

有关详细信息，请参阅[配合使用 Azure Data Lake Storage 和 Azure Databricks](/databricks/data/data-sources/azure/azure-datalake)。
...
如果你未创建工作区，但要添加为用户，请联系创建工作区的人员。 让他通过 Azure Databricks 管理员控制台添加。 有关说明，请参阅 [Adding and managing users](/databricks/administration-guide/users-groups/users)（添加和管理用户）。 如果已创建该工作区但仍出现此错误，请尝试再次在 Azure 门户中单击“初始化工作区”。
...
Databricks 群集为每个节点（包括驱动程序节点）使用一个公共 IP 地址。 对每个区域，Azure 订阅均具有[公共 IP 地址限制](/azure-resource-manager/management/azure-subscription-service-limits#publicip-address)。 因此，如果群集创建和纵向扩展操作导致分配给该区域中的该订阅的公共 IP 地址数量超过限制，则群集创建和纵向扩展操作可能会失败。 此限制还包括分配给非 Databricks 使用的公共 IP 地址，例如自定义用户定义的 VM。
...
- [请求提高公共 IP 地址限制](/azure-portal/supportability/resource-manager-core-quotas-request)。 选择“配额”作为“问题类型”、选择“网络: ARM”作为“配额类型”。 在“详细信息”中，请求增加公共 IP 地址配额。 例如，如果限制当前为 60，但希望创建具有 100 个节点的群集，则请求将限制增加至 160。
...
有关详细说明，请参阅[资源提供程序和类型](/azure-resource-manager/management/resource-providers-and-types)。
