...
若要启用运行容器工作负荷的 Batch 池，必须在池的 [VirtualMachineConfiguration](https://docs.azure.cn/dotnet/api/microsoft.azure.batch.virtualmachineconfiguration) 对象中指定 [ContainerConfiguration](https://docs.azure.cn/dotnet/api/microsoft.azure.batch.containerconfiguration) 设置。 （本文已提供 Batch .NET API 参考文章的链接。 [Batch Python](https://docs.microsoft.com/python/api/overview/azure/batch) API 中提供了相应的设置。）
